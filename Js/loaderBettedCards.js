export function createBettedCard(betData, onClick) {
    const tarjeta = document.createElement("article");
    tarjeta.className = "tarjeta-evento";
    tarjeta.innerHTML = `
        <img src="${betData.img}" alt="Evento">
        <div class="tarjeta-info">
            <h3>${betData.nombre}</h3>
            <p>${betData.categoria}</p>
        </div>
        <div class="tarjeta-porcentajes">
            <h3 class="porcentaje">${betData.porcentaje || ''}</h3>
    ...
    `;
    tarjeta.addEventListener("click", () => onClick(betData));
    return tarjeta;
}

export function generateBettedCards(contenedor, eventos, onClick, onSi, onNo) {
    eventos.forEach(evento => {
        const tarjeta = createBettedCard(evento, onClick);
